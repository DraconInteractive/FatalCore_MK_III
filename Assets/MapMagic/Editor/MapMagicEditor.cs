...
//			Undo.RecordObject (script, "MapMagic settings change");
			...
			if (script.guiSettings)
			{
				...
				if (MapMagic.instance.generateInfinite)
				{
					...
					layout.Field(ref MapMagic.instance.enableRange, "Enable Range", max:MapMagic.instance.generateRange);
			...
			if (layout.change) 
				foreach (Chunk tw in MapMagic.instance.terrains.Objects()) tw.SetSettings(script);
