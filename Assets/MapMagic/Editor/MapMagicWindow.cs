	...
	public class MapMagicWindow : EditorWindow
	{
		...
		#region Right-click actions
			...
			void PreviewOutput (Generator gen, Generator.Output output, bool inWindow)
			{
				...
				if (gen!=null) MapMagic.instance.guiGens.ChangeGenerator(gen);
				else 
				{
					SplatOutput firstSplatOutput = null;
					foreach (SplatOutput splatOut in MapMagic.instance.gens.GeneratorsOfType<SplatOutput>()) { firstSplatOutput = splatOut; break; }
					MapMagic.instance.guiGens.ChangeGenerator(firstSplatOutput);
					//TODO: maybe use SplatOutput.Process() here?
				}
				...
				/*PreviewOutput previewOutput = MapMagic.instance.guiGens.GetGenerator<PreviewOutput>();
				
				//removing an old one preview generator
				if (previewOutput != null)
					MapMagic.instance.guiGens.DeleteGenerator(previewOutput);

				//do nothing else if it is a 'clear' command (gen is null)
				if (gen==null || output==null) { PreviewWindow.CloseWindow(); MapMagic.instance.guiGens.ChangeGenerator(null); return; }
				
				//creating new preview and connecting it
				previewOutput = (PreviewOutput)MapMagic.instance.guiGens.CreateGenerator(typeof(PreviewOutput), gen.guiRect.position + new Vector2(200,10));
				...
				MapMagic.instance.guiGens.ChangeGenerator(previewOutput);*/
		...
		private void DrawWindow()
		{
			...
			//unity 5.4 beta
			if ((Event.current.type == EventType.MouseDrag && Event.current.button != 0) || Event.current.type == EventType.layout) return; 
				...
				foreach(Generator gen in gens.list)
				{
					foreach (Generator.Input input in gen.Inputs())
					{
						...
						{ 
							...
							if (portal.form == Portal.PortalForm.Out && 
								portal.input.linkGen != null &&
								!((Portal)portal.input.linkGen).drawConnections) 
								continue; //do not draw connections for exit portals (if it's input toggle is off)
						}
		...
		public void DrawPortalSelector (Portal exit, Generator.InoutType type)
		{
			...
			for (int g=0; g<gens.list.Length; g++)
			{
				...
				popupItems[counter] = new PopupMenu.MenuItem( enter.name, delegate () { exit.input.Link(enter.output, enter); gens.ChangeGenerator(exit); } );
				counter++;
			}
