...
//				MapMagic.instance.ForceGenerate();
	...
	public class Portal : Generator
	{
		...
		public override void OnGUI ()
		{
			...
			layout.Field(ref type, rect:layout.Inset(0.48f));
			if (type != input.type) { input.Unlink(); input.type = type; output.type = type; }

			layout.Field(ref form,rect:layout.Inset(0.35f));
			if (form == PortalForm.Out && 
				layout.Button("", layout.Inset(20), monitorChange:false, icon:"MapMagic_Focus_Small", tooltip:"Focus on input portal") && 
				input.linkGen != null)
			{
