...
namespace MapMagic 
{
		...
		public class Chunk
		{
			...
			public void CheckSetNeighbors ()
			{
				...
				if (oldNeig_x != terrains.GetTerrain(coord.x-1, coord.z, onlyComplete:false) ||
					oldNeig_Z != terrains.GetTerrain(coord.x, coord.z+1, onlyComplete:false) ||
					oldNeig_X != terrains.GetTerrain(coord.x+1, coord.z, onlyComplete:false) ||
					oldNeig_z != terrains.GetTerrain(coord.x, coord.z-1, onlyComplete:false) )
				{
					terrain.SetNeighbors( //terrainPrevX, terrainNextZ, terrainNextX, terrainPrevZ);
						terrains.GetTerrain(coord.x-1, coord.z, onlyComplete:false),
						terrains.GetTerrain(coord.x, coord.z+1, onlyComplete:false),
						terrains.GetTerrain(coord.x+1, coord.z, onlyComplete:false),
						terrains.GetTerrain(coord.x, coord.z-1, onlyComplete:false) );
				}
			}
			...
			public void Update (float camDist)
			{
				...
				/*Rect chunkRect = new Rect(coord.x*MapMagic.instance.terrainSize, coord.z*MapMagic.instance.terrainSize, MapMagic.instance.terrainSize, MapMagic.instance.terrainSize);
				
				bool inEnableRect = false;
				bool inGenerateRect = false;

				for (int i=0; i<camPoses.Length; i++)
				{
					inEnableRect = inEnableRect || chunkRect.Intersects( camPoses[i].ToRect(MapMagic.instance.enableRange) );
					inGenerateRect = inGenerateRect || chunkRect.Intersects( camPoses[i].ToRect(MapMagic.instance.generateRange) );
				}

				if (MapMagic.instance.terrains.IsNailed(coord)) inGenerateRect = true; //consider pinned terrains always in generate range
				if (MapMagic.instance.isEditor) inEnableRect = true; //consider all terrains in enable rect in editor */
				...
				//starting generate
				if (camDist<MapMagic.instance.generateRange && !running && clear) start = true;
				...
				lock (locker) while (true)
				{
					...
					try 
					{
						...
						if (MapMagic.instance.previewOutput != null) //generating preview
						{
							...
							if (!stop) previewObject = results[MapMagic.instance.previewOutput];
						}
						...
						if (changedTypes.Contains(typeof(HeightOutput))) HeightOutput.Process(this);
			...
			public IEnumerator ApplyRoutine ()
			{
				...
				if (!existingOutputs.Contains(typeof(SplatOutput)) && terrain.terrainData.alphamapResolution != 16) SplatOutput.Purge(this);
...
/*				if (instance.previewOutput != null && previewObject != null) 
			...
			public void Preview (bool forceRefresh=false)
			{
				...
				//previewing matrix
				SplatPrototype[] prototypes = terrain.terrainData.splatPrototypes;
				//bool previewApplied = !(prototypes==null || prototypes.Length<2 || prototypes[0].texture==null || prototypes[0].texture!=redPrototype.texture);
				if (previewObject is Matrix && forceRefresh)
				{
					//preparing prototypes
					if (prototypes==null || prototypes.Length<2) prototypes = new SplatPrototype[Mathf.Max(prototypes.Length,2)];
					prototypes[0] = redPrototype; prototypes[1] = greenPrototype;

					//preparing out
					Matrix matrix = (Matrix)previewObject;
					float[,,] heights3D = new float[matrix.rect.size.x, matrix.rect.size.z, Mathf.Max(prototypes.Length,2)];
					...
					data.alphamapResolution = heights3D.GetLength(0);
					data.SetAlphamaps(0,0,heights3D);
				}

				if (previewObject is SpatialHash)
				{
					try 
					{
						float pixelSize = 1f * MapMagic.instance.terrainSize / MapMagic.instance.resolution;
						SpatialHash objs = (SpatialHash)previewObject;
						foreach (SpatialObject obj in objs)
						{
							...
							Gizmos.DrawLine(pos+new Vector3(obj.size/2f,0,0), pos-new Vector3(obj.size/2f,0,0));
							Gizmos.DrawLine(pos+new Vector3(0,0,obj.size/2f), pos-new Vector3(0,0,obj.size/2f));
							...
							Gizmos.DrawLine(new Vector3(pos.x, 0, pos.z), new Vector3(pos.x, MapMagic.instance.terrainHeight, pos.z));
						}
					}
					catch {}
				}
